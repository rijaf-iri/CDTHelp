...
`r put.image("images/data_prepraration/format_cdt_data/fill_dek_temp.gif", height = 50, width = 55)`

1. Select from the drop-down list the file (in CDT station data format) containing the dekadal temperature data to be filled if it is already loaded, or open it from the browse button ![][browsebutf] on the left.
2. Type the full path to the folder containing the adjusted reanalysis files (in NetCDF format) or browse it from the button ![][browsebutf] on the left. Do not forget to provide a sample file and change the filename format of the NetCDF file by clicking on the button `r button('Settings')` (see [Setting NetCDF data](cdt_data_input.html#cdt.ncdf.setting) for more details).
...
5. Type the name of the file to save the filled data or the full path to the file. You can also click on ![][browsebutf] on the left, it opens a file dialog where you can select and/or type the name, the path and the file format (txt or csv).
